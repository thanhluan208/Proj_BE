...
export class ContractsService {
  ...
  async generateContractData(
    ...
    const contractData = {
      ...
      roomElectricFee: Number(room.electronic_fee).toLocaleString('vi-VN'),
      roomInternetFee: '100,000', // Fixed value, will be configurable later
      roomWaterFee: Number(room.water_fee).toLocaleString('vi-VN'),
      roomCleaningFee: '50,000', // Fixed value
  ...
  async generateContract(
    ...
    try {
      ...
      if (saveToFile) {
        filePath = await this.saveContractToFile(buffer, contractData);
      }
  ...
  private saveContractToFile(
    buffer: Buffer,
    contractData: any,
  ): Promise<string> {
    try {
      // Create files directory if it doesn't exist
      const filesDir = path.join(process.cwd(), 'src', 'contracts', 'files');
      ...
      this.logger.log(`Contract saved to: ${filePath}`);
  ...
  async generateAndSaveContract(
    ...
    return result as { buffer: Buffer; filePath: string };
  }
